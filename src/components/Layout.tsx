      ...
      <aside className="fixed top-0 left-0 h-screen z-50 flex flex-col
        ...
        <div className="px-2 lg:px-4 py-4 border-b dark:border-gray-800 flex items-center justify-center lg:justify-start gap-2.5 shrink-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0"
            style={{ background: "linear-gradient(135deg,#c0392b,#8e1a10)" }}>G</div>
          ...
          {NAV.filter(item => !item.role || user.role === item.role || user.role === "super_admin").map(item => {
            ...
            return (
              <Link key={item.href} to={item.href} title={item.label}
                className={`flex items-center justify-center lg:justify-start gap-0 lg:gap-2.5
                  px-2 lg:px-3 py-2.5 lg:py-2 rounded-lg text-sm font-medium transition-all
                  ${active
        ...
        <div className="px-1 lg:px-3 py-2 border-t dark:border-gray-800 shrink-0">
          <button onClick={toggle} title={dark ? "Light Mode" : "Dark Mode"}
            className="w-full flex items-center justify-center lg:justify-start gap-0 lg:gap-2.5
              px-2 lg:px-3 py-2 rounded-lg text-sm font-medium
              text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-800 dark:hover:text-gray-100 transition-all">
            {dark ? (
              <>
        ...
        <div className="px-1 lg:px-3 py-3 border-t dark:border-gray-800 shrink-0">
          <div className="flex flex-col lg:flex-row items-center gap-2 lg:gap-2.5">
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
              style={{ background: "linear-gradient(135deg,#c0392b,#8e1a10)" }}>{initials}</div>
            ...
            <div className="flex gap-2 lg:gap-1">
              <Link to="/change-password" title="Change Password"
                className="text-gray-300 dark:text-gray-600 hover:text-blue-400 dark:hover:text-blue-400 transition-colors p-1">
      ...
      <main className="flex-1 flex flex-col min-w-0 ml-14 lg:ml-52">
        ...
        <header className="bg-white dark:bg-gray-900 border-b dark:border-gray-800 px-4 py-3 flex items-center justify-between shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border dark:border-gray-700 rounded-lg px-3 py-1.5">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold hidden sm:block ${
              user.role === "super_admin" ? "bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400"
              : user.role === "manager"   ? "bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400"
